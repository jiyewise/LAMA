...
SkeletonBuilder::
buildFromFile(std::string _xml, bool _isScene, std::string _objPath) {
	TiXmlDocument doc;
	...
	for(TiXmlElement *body = skeldoc->FirstChildElement("Joint"); body != nullptr; body = body->NextSiblingElement("Joint")){
		...
		// shape
		std::string shape = "";
		if(body->Attribute("shape") != nullptr) {
			shape = body->Attribute("shape");
		}

		if(!jointType.compare("FreeJoint") ){
			SkeletonBuilder::makeFreeJointBody(skel, parent,
											   name, size, mass,
											   jointPosition, bodyPosition, shape);
		} 
			...
			SkeletonBuilder::makeBallJointBody(skel, parent, 
											   ...
											   isLimitEnforced, upperLimit, lowerLimit, shape);
		} 
		else if(!jointType.compare("WeldJoint") && !_isScene){
			...
			if(body->Attribute("axis") != nullptr)
				axis = str2Vec3d(body->Attribute("axis"));

			SkeletonBuilder::makeRevoluteJointBody(skel, parent,
													name, size, mass,
													jointPosition, bodyPosition,
													isLimitEnforced, upperLimit, lowerLimit, shape, axis);
		}
		// else if(!jointType.compare("WeldJoint") && _isScene){
		// 	SkeletonBuilder::makeSceneJointBody(skel, parent, 
		// 									   name, size, mass,
		// 									   jointPosition, bodyPosition, _objPath);			
		// }
	...
	std::string _shape)
{
	...
	if(_shape != "") {
		std::string objPath = LAMA_DIR + std::string("/data/scene/") + _shape;
		const aiScene* mesh = dart::dynamics::MeshShape::loadMesh(std::string(objPath));

		dart::dynamics::ShapePtr visualShape = std::shared_ptr<dart::dynamics::MeshShape>(new dart::dynamics::MeshShape(Eigen::Vector3d(1,1,1),mesh));
		auto vsn = bn->createShapeNodeWith<dart::dynamics::VisualAspect,dart::dynamics::CollisionAspect,dart::dynamics::DynamicsAspect>(visualShape);
	...
	std::string _shape)
{
	...
	// this is for physics simulation
	...
	if(_isLimitEnforced){
		...
		joint->setPositionLimitEnforced(_isLimitEnforced);
	...
	const Eigen::Vector3d& _axis)
{
	...
	bn = _skel->createJointAndBodyNodePair<dart::dynamics::RevoluteJoint>(
		_parent, props, dart::dynamics::BodyNode::AspectProperties(_name)).second;
	...
	if(_isLimitEnforced){
		dart::dynamics::JointPtr joint = bn->getParentJoint();
		joint->setPositionLimitEnforced(_isLimitEnforced);
		joint->setPositionUpperLimit(0, _upperLimit);
		joint->setPositionLowerLimit(0, _lowerLimit);
	}
...
// 	const aiScene* mesh = dart::dynamics::MeshShape::loadMesh(std::string(_objPath));

// 	dart::dynamics::ShapePtr visualShape = std::shared_ptr<dart::dynamics::MeshShape>(new dart::dynamics::MeshShape(Eigen::Vector3d(1,1,1),mesh));
// 	auto vsn = bn->createShapeNodeWith<dart::dynamics::VisualAspect,dart::dynamics::CollisionAspect,dart::dynamics::DynamicsAspect>(visualShape);
// 	Eigen::Isometry3d T_visual;
// 	T_visual.setIdentity();
// 	Eigen::Vector3d pos(0, 1.62, 0);
// 	T_visual.translation() = pos;
