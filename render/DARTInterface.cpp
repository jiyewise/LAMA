...
void
GUI::
drawSkeleton(const dart::dynamics::SkeletonPtr& _skel, int _type, bool _drawMeshColor)
{
	for(int i = 0; i < _skel->getNumBodyNodes(); i++)
	{
		...
		drawShape(T, shapeNodes[_type]->getShape().get(), shapeNodes[_type]->getVisualAspect()->getRGBA(), _drawMeshColor);
	}
}

void
GUI::
drawSkeleton(const dart::dynamics::SkeletonPtr& _skel, std::map<std::string, Eigen::Vector4d> _colorMap, int _type)
{
	for(int i = 0; i < _skel->getNumBodyNodes(); i++)
	{
		auto bn = _skel->getBodyNode(i);
...
drawShape(Eigen::Isometry3d _T,
	...
	Eigen::Vector4d _color, bool _drawMeshColor)
{
	...
	else if(_shape->is<MeshShape>())
	{
		auto* mesh = dynamic_cast<const MeshShape*>(_shape);
    	...
    	GUI::drawMesh(mesh->getScale(),mesh->getMesh(), _color, _drawMeshColor);
